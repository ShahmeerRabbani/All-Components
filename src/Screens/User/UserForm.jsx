...
const UserForm = () => {
    ...
    const handleSubmitRegister = async (e) => {
      e.preventDefault();
     if(firstName.trim() === '' || lastName.trim() === '' || email.trim() === '' || age.trim() === 0 || room.trim() === 0 || radio === ''){
      alert('Please fill all value')
     }
     else{
      try {
        const addRegister = addDoc(collection(db, "UserInformation"), userRegistration)
          .then((response) => {
  ...
  return (
      ...
      <form className='Content'>
        ...
        <InputField onchange = {(e) => setRoom(e.target.value)} label='Room' placeholder='Enter your Room' type='number'/>
        <p>
